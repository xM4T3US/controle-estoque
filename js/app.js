...
(function () {
  ...
  const App = {
    ...
    async init() {
      ...
      // Verifica autenticação ANTES de liberar o app
      const st = await Auth.status();
      if (st.online && !st.configurado) {
        // Servidor sem senha definida ainda
        this.showLogin({
          hint:
            "Nenhuma senha foi definida ainda. No servidor, abra <code>definir-senha.php</code> para criar a senha e depois apague esse arquivo.",
        });
        return;
      }
      if (st.online && !st.auth) {
        // Online mas sem sessão -> exige login
        this.showLogin();
        return;
      }
      if (!st.online && !Auth.hadSession()) {
        // Offline e nunca logou neste aparelho -> não há cache confiável
        this.showLogin({
          hint:
            "Você está sem conexão. Conecte-se à internet para entrar pela primeira vez.",
        });
        return;
      }

      // Autenticado (ou offline com sessão anterior) -> liberar o app
      await this.bootApp();
    },

    /** Carrega de fato o app depois de autenticado. */
    async bootApp() {
      if (this._booted) {
        this.hideLogin();
      ...
      this.hideLogin();
      await Storage.init();
      if (Storage.syncFromServer) {
        try { await Storage.syncFromServer(); } catch (e) { /* offline ok */ }
      }
      this.bindEvents();
    ...
    /* ================= LOGIN (UI) ================= */
    bindLoginEvents() {
      // Se a sessão cair durante o uso, volta para a tela de login
      window.addEventListener("ce:unauthorized", () => {
        if (this._booted) this.showLogin({ hint: "Sua sessão expirou. Entre novamente." });
      });
      const btn = document.getElementById("loginBtn");
      const input = document.getElementById("loginSenha");
      const toggle = document.getElementById("loginToggle");
      if (btn) btn.addEventListener("click", () => this.fazerLogin());
      if (input) {
    ...
    loginMsg(texto, tipo) {
      ...
      if (!m) return;
      m.className = "login-msg " + (tipo === "ok" ? "is-ok" : "is-error");
      m.textContent = texto;
    ...
    async fazerLogin() {
      ...
      if (!senha) { this.loginMsg("Digite a senha.", "err"); return; }
      if (btn) { btn.disabled = true; btn.innerHTML = '<i class="bi bi-hourglass-split"></i> Entrando...'; }
      const r = await Auth.login(senha);
      if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-unlock-fill"></i> Entrar'; }
      if (r.ok) {
        const m = document.getElementById("loginMsg");
        if (m) m.classList.add("d-none");
        await this.bootApp();
      } else if (r.configurado === false) {
        this.loginMsg("Senha não configurada no servidor.", "err");
        this.showLogin({
          hint:
            "No servidor, abra <code>definir-senha.php</code> para criar a senha e depois apague esse arquivo.",
        });
      } else {
        this.loginMsg(r.erro || "Senha incorreta.", "err");
      }
    },
    ...
    async logout() {
      const { Auth } = CE;
      await Auth.logout();
      // recarrega para limpar estado da tela e mostrar login
    ...
    atualizarPreviewDiff() {
      ...
      this.toggleObs(diff !== 0);
    },

    // mostra o campo de observação apenas quando há divergência
    toggleObs(mostrar) {
      const wrap = document.getElementById("f-obs-wrap");
      if (!wrap) return;
      wrap.classList.toggle("d-none", !mostrar);
      if (!mostrar) {
        const obs = document.getElementById("f-obs");
        if (obs && !this.editId) obs.value = "";
      }
    },
    ...
    async salvarRegistro() {
      ...
      const diferenca = Number(fisico) - Number(sistema);
      const dados = {
        rua,
        produto,
        ...
        // observação só faz sentido quando há divergência
        observacao: diferenca !== 0 ? Utils.sanitize(obs) : "",
      };
    ...
    /* ================= TABELA ================= */
    async refreshAll() {
      // Re-sincroniza do servidor (fonte da verdade) quando online,
      // para que edições/exclusões reflitam em TODAS as telas na hora.
      if (Storage.syncFromServer) {
        try { await Storage.syncFromServer(); } catch (e) { /* offline: usa local */ }
      }
      this.cacheRegistros = await Storage.query({});
      // Redesenha sempre a tabela principal e o dashboard
      this.renderTabela();
      this.renderDashboard();
      // Redesenha a view ativa, se for uma que mostra lista de registros
      if (this.activeView === "historico") this.renderHistorico();
      // Se houver uma pesquisa de rua ativa, re-executa para atualizar
      // o resumo, os resultados e a "Auditoria da Rua".
      if (this.ultimaPesquisaRua && document.getElementById("p-rua").value.trim()) {
        try { await this.executarPesquisa(); } catch (e) { /* ignora */ }
      }
      // Atualiza também a lista do modal de edição, se estiver aberto
        ...
        ? res
            .map((r) => {
              ...
              <div class="tl-body">
                ...
                r.observacao
                  ? `<div class="tl-obs"><i class="bi bi-chat-left-text"></i> ${Utils.escapeHtml(
                      r.observacao
                    )}</div>`
                  : ""
              }
